...
resource "aws_kms_key" "cloudwatch" {
  description             = "CMK for the Snowplow Collector Cloudwatch Log Group"
...
data "aws_iam_policy_document" "cloudwatch_kms_policy" {
  statement {
    ...
    principals {
      type = "AWS"
      identifiers = [
        data.aws_iam_role.snowplow_collector.arn
      ]
    }
    actions = [
      "kms:Decrypt",
...
resource "aws_iam_policy" "kms_decrypt_cloudwatch" {
  ...
  path        = "/"
  description = "This Policy gives access to decrypt snowplow collector cloudwatch KMS"
